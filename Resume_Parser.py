...
# Read job description from job_description.txt

job_description_file = Path("job_description.txt")

if not job_description_file.exists():
    raise FileNotFoundError(
        "job_description.txt file not found"
    )
...
system_prompt = f"""
you are an expert HR assistent.

your job is to analyze job descriptions and expract
and exptract structured information from them.

Return only valid JSON matching this schema:

{jobd_schema}
IMPORTANT:
Do NOT return the scema itself
Do NOT return fields like "properties", "title" or "type".
Fill the schema with actual information extracted from the job descriptions.
...
def final_score(job, resume):
    ...
    5.Overll match percentage from 0 to 100
...
for candidate in top_2:
    ...
    print("Experience Requirement Met:", candidate["experience_requirement_met"])
    print("Verdict:", candidate["final_verdict"])
